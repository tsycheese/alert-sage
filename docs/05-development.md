...
V2.7C 不增加新的业务能力，而是把已验证能力组织成招聘方可以在五分钟内理解的证据链。根目录 README 提供问题定位、五条工程亮点和人工确认主图；`docs/09-portfolio-showcase.md` 保存系统架构图、持久化恢复时序、分段讲解脚本、能力证据映射、常见追问与离线备用路径。

截图通过独立 `showcase:capture` Playwright 配置生成，复用 V2.7B 的 Mock 供应商、受限 `logs` 故障注入、专用端口和临时 Volume。共享的表单助手与浏览器配置位于 `tests/playwright/`，业务回归和展示采集不再重复维护表单标签及浏览器基线。采集只在浏览器页面中临时隐藏粘性导航、消息浮层和截图外的时间线区域，不修改生产 UI。

三张版本化图片分别证明结构化报告与人工门控、持久化时间线中的工具部分失败，以及批准后的案例同步。截图文档明确标识为 Mock 演示；真实 Dify/DeepSeek 和 RAG 评测仍以既有运行记录与评测报告为证据，展示流程不会伪造云端结果。

2026-07-28 在全新临时数据卷中完成资产重生成，Playwright 采集耗时 6.8 秒；三张 PNG 逐张按原始分辨率检查，卡片边距、文字清晰度、失败状态和同步状态均正确，且没有导航或消息浮层遮挡。共享测试代码抽取后，V2.7B 主路径再次运行通过，耗时 6.7 秒。V2.7C 质量门为后端 107 项测试、前端 14 项测试、Ruff、Ruff 格式检查、Python 编译、TypeScript 类型检查、生产构建、PowerShell 语法、Playwright 主路径和展示采集全部通过。
