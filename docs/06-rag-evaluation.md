...
1. `/evaluations`：按 split、状态筛选运行，展示参数、状态、进度和核心指标，并选择两条兼容运行。
2. `/evaluations/{run_id}`：展示汇总指标、评测集哈希、构建 revision、运行参数和逐题检索排名。
3. `/evaluations/compare?left={id}&right={id}`：展示右侧相对左侧的指标与逐题变化。

只有同时满足以下条件的运行可以比较：两者都为 `completed`，且评测集 ID、版本、SHA-256 和 split 完全一致。TopK、阈值与 provider 允许不同，因为它们属于实验变量。列表复选框会提前禁用不兼容项，对比页仍再次执行相同校验，避免通过手写 URL 绕过。

页面通过 React Query 管理服务端状态：列表仅在当前页存在活动运行时轮询，详情仅在目标运行为 `queued` 或 `running` 时轮询，完成后停止。筛选参数和对比运行 ID 保存在 URL；不引入新的全局状态库，也不增加数据表。逐题结果只展示后端已经脱敏并归一化的排名、文档名、分数和来源。

V2.6C 保持只读，不提供在线编辑器、自动参数选择或 test 启动按钮。这样既能展示 calibration 阈值从无到 0.40 后拒答指标的改善，也不会让报告页面成为污染保留集的入口。
